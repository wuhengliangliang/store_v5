...
public class OrderServlet extends BaseServlet {
	//saveOrder; 保存订单的方法，就是将购物车中的信息以订单的形式保存
	public String saveOrder(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		// 确认用户是登录状态
	User user = (User)req.getSession().getAttribute("loginUser");
		if(user==null) {
			req.setAttribute("msg", "请登录之后再去神操作一把");
			return "/jsp/infor.jsp"; //没哟登录之后直接返回
		}
		//获取购物车
		Cart cart = (Cart)req.getSession().getAttribute("cart");
		// 创建订单项
		Order order = new Order();
		order.setOid(UUIDUtils.getCode()); //拿到生成的随机码
		...
		for (CartItem item : cart.getCartItems()) {
			...
			//设置当前的订单项是属于哪个订单的，程序的角度体验订单项和订单对应的关系
			orderItem.setOrder(order);
			order.getList().add(orderItem);  //将订单项加入到订单上的集合当中
		}
		// 创建业务逻辑层功能：保存订单
		OrderService OrderService = new OrderServiceImp();
		//将订单数据，用户的数据，订单下面所有的订单项都传到service层
		OrderService.saveOrder(order);
		// 清空购物车
		cart.clearCart();
		// 将订单放入request
		req.setAttribute("order", order); //将订单项传入 /jsp/order_info.jsp 这个界面当中
		// 转发到/jsp/order_info.jsp下面
	...
	public String findMyOrdersWithPage(HttpServletRequest req, HttpServletResponse resp) throws Exception{
		...
		//PageModel :携带的参数 1，分页参数2，url 3,当前用户的当前页的订单（集合），每笔订单上对应的订单项，以及订单项对应的商品信息
